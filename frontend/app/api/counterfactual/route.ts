...
Each visual_prompt must read like a realistic driving simulation seen from just behind the silver ego car on a busy three-lane highway, camera riding with that car. IMPORTANT: because the camera moves with the ego car, you cannot show it slowing by moving it in frame. Describe deceleration by what is visible instead - the ego car's brake lights coming on, and the road surface and lane markings rushing past the camera slowing right down until they are still. Likewise show acceleration or steady speed by how fast the road flows past. Structure it as: road flowing, then the event, then the ego's visible reaction. The white van sits two car lengths ahead; other cars drive in the lanes on both sides. For BRAKE, say the side lanes are busy so there is no room to swerve. Keep it short and factual. Never use the words drone, aerial or cinematic. No camera moves, no zoom, no slow motion.
        ...
        counterfactuals: [
          {
            ...
            safety_score: 87,
            fatalities: false,
            visual_prompt: `A realistic driving simulation seen from just behind a silver car on a busy three-lane highway. At first the road rushes past quickly. Then the white van two car lengths ahead switches on bright red brake lights and stops dead. The silver car's brake lights come on, the road and lane markings rushing past the camera slow right down to a complete standstill, and it ends up stopped just behind the van. The side lanes are busy so there is no room to swerve. ${s}... The car's brake lights glow red, its nose dips under heavy deceleration, and the gap to the van closes. The car stops safely with meters of clearance. Camera fixed to the silver car and moving at exactly its speed so it holds the same spot in frame, the white van a couple of car lengths ahead in the same lane, several other cars in the neighbouring lanes. Clear daylight.`,
          },
          {
            action: "TURN",
            safety_score: 52,
